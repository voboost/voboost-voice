...
class PhoneCallContactIntentHandler(context: Context) : AbstractIntentHandler("phone_call_contact", context) {
    ...
    override fun buildIntent(config: ActionConfig, voiceParams: Map<String, Any>): android.content.Intent {
        ...
        return android.content.Intent(ACTION_IVOKA_PHONE_CALL).apply {
            // Если номер найден - отправляем его, иначе имя контакта (BluetoothPhone попытается набрать)
            putExtra(EXTRA_IVOKA_CALL_INFO, phoneNumber ?: contactName)
            putExtra(EXTRA_SCREEN_INT, 0)
            putExtra(EXTRA_MAC, CALL_PARAM_NUMBER)
        }
    }
    ...
    private fun findPhoneNumberByName(contactName: String): String? {
        val bluetoothMac = getBluetoothMac()
            ?: return null.also { Log.e(TAG, "Cannot get Bluetooth MAC address") }
    ...
    private fun getBluetoothMac(): String? {
        return try {
            ...
            method.invoke(null, "qinggan.bluetooth.mac") as? String
        } catch (e: Exception) {
