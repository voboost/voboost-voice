...
//        return paramNames
    ...
    fun isConfirmationYes(text: String, commandConfig: CommandConfig): Boolean {
        ...
        val defaultYesPatterns = listOf(
            "да", "ага", "угу", "подтверждаю", "ок", "открывай",
            "давай", "конечно", "ага да", "yes", "yeah"
        )
    ...
    fun isConfirmationNo(text: String, commandConfig: CommandConfig): Boolean {
        ...
        // Паттерны по умолчанию
        val defaultNoPatterns = listOf(
            "нет", "не надо", "не нужно", "отмена", "отмени",
            "не", "отбой", "стоп", "no", "nah"
        )
    ...
    fun getConfirmationQuestion(commandConfig: CommandConfig): String {
        return commandConfig.confirmation.question 
            ?: "Подтверждаете выполнение команды?"
    }
