...
class VoskRecognitionEngine(private val recognizer: Recognizer)
     ...
     */
    override fun acceptWaveform(pcm: ByteArray, start: Int, end: Int): RecognitionEngineResult? {
            ...
            return try {
                if (recognizer.acceptWaveForm(chunk, chunkSize)) { // Финальный результат
                    ...
                    if (partialText.isNotEmpty()) {
                        ...
                        RecognitionEngineResult(text = partialText, isPartial = true)
                    }
                    else null
                }
            }
     ...
     */
    override fun getFinalResult(): RecognitionEngineResult? {
