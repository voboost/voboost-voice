...
class SpeechService(private val speechEngine: ISpeechEngine) : ISpeechServiceCallback,
    ...
    init {
        speechEngine.addCallback(this)
    }

    /**
     * Обычный enqueue — не ждёт завершения
     */
    override fun enqueue(text: String, priority: Int) {
     ...
     */
    override suspend fun enqueueAsync(text: String, priority: Int): Boolean {
    ...
    private fun enqueue(item: SpeechItem) {
        synchronized(queueLock) { // Если очередь пуста и ничего не говорится — запускаем сразу
    ...
    private fun startNextItem() {
        synchronized(queueLock) {
            ...
            speechEngine.speak(item.text) // ← Не вызываем completion здесь! Ждём колбэк от движка
        }
    }
