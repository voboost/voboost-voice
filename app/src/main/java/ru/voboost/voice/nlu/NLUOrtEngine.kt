...
class NLUOrtEngine(context: Context, private val configManager: ConfigManager) : INLUEngine {
    ...
    companion object {
        ...
        private const val SIMILARITY_THRESHOLD = 0.62f // ╨Я╨╛╤А╨╛╨│ ╤Б╤А╨░╨▒╨░╤В╤Л╨▓╨░╨╜╨╕╤П
        private const val EMBEDDING_DIM = 384 // ╨а╨░╨╖╨╝╨╡╤А╨╜╨╛╤Б╤В╤М MiniLM-L12
    }
    ...
    init {
        ...
        Log.i(TAG, "тЬЕ ONNX NLU initialized. Commands indexed: ${commandPatterns.size}")
    }
    ...
    override fun parseCommand(text: String): RecognizedCommand? {
        return try {
            ...
            if (bestScore >= SIMILARITY_THRESHOLD && bestId != null) {
                Log.d(TAG,
                      "тЬЕ Matched: '$text' тЖТ $bestId (score: ${String.format("%.3f", bestScore)})")
                val params = extractParams(bestPattern!!, text)
                buildRecognizedCommand(bestId, params)
            }
            else {
                Log.d(TAG, "тЭМ No match for: '$text' (best: ${String.format("%.3f", bestScore)})")
                null
            }
        }
        catch (e: Exception) {
    ...
    private fun embedText(text: String): FloatArray {
        ...
        return try {
            ...
            finally { // ЁЯФе ╨Ч╨░╨║╤А╤Л╨▓╨░╨╡╨╝ ╨Т╨б╨Х ╤А╨╡╤Б╤Г╤А╤Б╤Л ╨▓ ╨╛╨▒╤А╨░╤В╨╜╨╛╨╝ ╨┐╨╛╤А╤П╨┤╨║╨╡
                outputTensor?.close()
                result?.close()
    ...
    private fun cosineSimilarity(a: FloatArray, b: FloatArray): Float {
        ...
        return dot // ╨Т╨╡╨║╤В╨╛╤А╨░ ╤Г╨╢╨╡ ╨╜╨╛╤А╨╝╨░╨╗╨╕╨╖╨╛╨▓╨░╨╜╤Л, ╨┐╨╛╤Н╤В╨╛╨╝╤Г dot product == cosine similarity
    }
    ...
    private fun extractParams(pattern: String, text: String): Map<String, String> {
        ...
        // 2. ╨н╨║╤А╨░╨╜╨╕╤А╤Г╨╡╨╝ ╨Т╨б╨Х ╤Б╨┐╨╡╤Ж╤Б╨╕╨╝╨▓╨╛╨╗╤Л regex, ╨║╤А╨╛╨╝╨╡ ╨╜╨░╤И╨╕╤Е {╨┐╨░╤А╨░╨╝╨╡╤В╤А╨╛╨▓}
        var regexPattern = Regex.escape(pattern) // ╤Н╨║╤А╨░╨╜╨╕╤А╤Г╨╡╨╝ ╨▓╤Б╤С
        for (param in paramNames) {
            // ╨Т╨╛╤Б╤Б╤В╨░╨╜╨░╨▓╨╗╨╕╨▓╨░╨╡╨╝ ╨╜╨░╤И╨╕ ╨│╤А╤Г╨┐╨┐╤Л: \{param\} тЖТ (?<param>[^}]+)
